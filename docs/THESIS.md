...
| **Status** | GitHub shipping candidate; architecture frozen pending final treaty-owner sign-off and canonical PR merge |
...
Grounded Distillery is a local model-development and deployment-gating system for progressively specializing a small language model on a measured agentic workload.
...
- eventual native Sovereign architecture.

Grounded evidence may inform Sovereign.

It does not automatically become Sovereign policy.
...
Current state: **STARTED_PARTIAL**. Real success and failure metadata have been imported without prompt/output content; remaining live recovery, implicit-signal, dashboard, alarm, and fresh-session acceptance remains to be proven.
...
- no G2/G3/G4 execution performed;
- no deployment or promotion performed.

## Hard-gate state

| Gate | Current state |
|---|---|
| HG-0 | **BLOCKED / PARTIAL LIVE** |
...
After repository integration review and PR authorization:
